...
export function collisionSystem(world: World, _dt: number): void {
  ...
  for (let i = 0; i < entities.length; i++) {
    ...
    if (world.hasComponent(eidA, Dead)) continue;
    ...
    for (let j = i + 1; j < entities.length; j++) {
      ...
      if (world.hasComponent(eidB, Dead)) continue;
 ...
 */
export function combatSystem(world: World, _dt: number): void {
  for (const cmd of world.getCommands("damage")) {
    ...
    if (!health) continue;
    if (world.hasComponent(targetId, Dead)) continue;
